...
<body class="bootstrap-admin-with-small-navbar">
					...
					<div class="col-lg-12">
						<div class="panel panel-default bootstrap-admin-no-table-panel">
							...
							<div
								...
								<form class="form-horizontal" action="/Demo_war_exploded/book?method=list"
									method="post">
					...
					<div class="col-lg-12">
						...
						<c:if test="${param.word==null}">
							...
							<jsp:include page="/page.jsp">
								<jsp:param value="book?method=list" name="url"/>
							</jsp:include>
							<br>
						</c:if>
						<c:if test="${param.word!=null}">
							<br>
							<jsp:include page="/page.jsp">
								<jsp:param value="book?method=list&word=${param.word}" name="url"/>
							</jsp:include>
							<br>
						</c:if>
	...
	<script type="text/javascript">
		...
		function deleteBook(bid) {
			con=confirm("是否删除?");
			$.post("book_delete",{bid:bid},function (data){
				if(data=="ok"){
					location.reload();
				}
			});
	...
	<!-- 修改模态框（Modal） -->
	<form class="form-horizontal" method="post" onsubmit="updBook()"
		action="/Demo_war_exploded/book?method=updateBook">
		<!--保证样式水平不混乱-->
						...
						<div class="form-group">
							...
							<div class="col-sm-7">
								<input type="text" class="form-control" id="updateBookName"
									name="bookName" placeholder="请输入图书名称" readonly="readonly"> <label
						...
						<div class="form-group">
							...
							<div class="col-sm-7">
								<input type="text" class="form-control" id="updateAutho"
									name="author" placeholder="请输入作者名称" readonly="readonly"> <label
									class="control-label" for="updateAutho" style="display: none;"></label>
							</div>
						</div>

						<div class="form-group">
							<label for="firstname" class="col-sm-3 control-label">出F版社</label>
							<div class="col-sm-7">
	...
	<!--------------------------------------添加的模糊框------------------------>
	<form class="form-horizontal" method="post" onsubmit="adBook()"
		action="/Demo_war_exploded/book?method=addBook">
		<!--保证样式水平不混乱-->
		<!-- 模态框（Modal） -->
	...
	<script>
		function adBook(){
			$.post("book_add",{bookName:document.getElementById('addBookName').value
			,typeName:document.getElementById('addBookType').value,author:document.getElementById('addAutho').value,
				num:document.getElementById('addNum').value,press:document.getElementById('addPress').value},function (data){
				if(data=="ok"){
		...
		function updBook(){
			//console.log(1)
			//alert(1);
			$.post("book_update",{typeName:document.getElementById('updateBookType').value
				,bid:document.getElementById("updateBookId").value,num:document.getElementById("updateNum").value,press:document.getElementById("updatePress").value},function (data){
				if(data=="ok"){
					location.reload();
				}
			});
