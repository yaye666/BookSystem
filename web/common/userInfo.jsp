...
<form class="form-horizontal" method="post" onsubmit="updPassword()"
	action="/Demo_war_exploded/user?method=updUserInfo">
	<!--保证样式水平不混乱-->
	<!-- 模态框（Modal） -->
	...
	<script>
		function updPassword(){
			con=confirm("是否修改?");
			$.post("password_change",{OldPassword:document.getElementById("oldPwd").value,NewPassword:document.getElementById("newPwd").value},function (data){
				if(data=="ok"){
					//location.reload();
					alert("密码修改成功，请重新登录")
					window.location.href="/Demo_war_exploded/login.jsp"
				}else if(data=="no"){
					alert("原密码错误，修改失败");
					location.reload();
				}
		...
		function updNAP(){
			...
			$.post("information_change",{name:document.getElementById("name").value,phone:document.getElementById("phone").value},function (data){
				if(data=="ok"){
					alert("修改成功！")
					location.reload();
...
<form class="form-horizontal" method="post" onsubmit="updNAP()"
	action="/Demo_war_exploded/user?method=list">
	<!--保证样式水平不混乱-->
	<!-- 模态框（Modal） -->
